...
class RotateSong extends ConsumerStatefulWidget {
  const RotateSong({
    super.key,
  ...
  @override
  ConsumerState<RotateSong> createState() => _RotateSongState();
}

class _RotateSongState extends ConsumerState<RotateSong>
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => navigateToDetail(songList[widget.currentIndex], context),
      onVerticalDragUpdate: (details) async {
        if (details.delta.dy <= 0) {
          if (!_rotationController.isAnimating) {
            if (widget.currentIndex < songList.length - 1) {
              setState(() {
      ...
      child: Hero(
        tag: songList[widget.currentIndex].pictureAlt!,
        flightShuttleBuilder: (flightContext, animation, flightDirection,
            fromHeroContext, toHeroContext) {
          return Image.asset(
            flightDirection.name == "push"
                ? "lib/assets/${songList[widget.currentIndex].pictureAlt}"
                : "lib/assets/${songList[widget.currentIndex].picture}",
